...
summary: Governs Axel's processing of personal data on the Customer's behalf, with GDPR/UK-GDPR and CCPA/CPRA terms, EU Standard Contractual Clauses, the security measures Axel implements, and the current list of sub-processors.
---
...
The subject matter, nature and purpose of the Processing, the categories of Data Subjects, and the types of Personal Data are described in **Annex I**. In summary, rolln processes Personal Data to provide the Service: receiving inbound webhook payloads at the edge; storing raw payloads for replay; queuing events; evaluating the Customer's declarative route, filter, and transform configuration; and fanning out deliveries to the Customer's chosen destinations, together with related logging, analytics, support, security, and billing functions.
...
- **Raw payloads** (Cloudflare R2): retained approximately 30 days by default for replay, configurable from 0 to 30 days.
- **Delivery / event logs** (ClickHouse): retained for an approximately 30-day time-to-live.
- **Dead-letter records:** retained approximately 90 days by default, configurable from 1 to 365 days.
- **Replay-request records:** retained approximately 30 days by default, configurable from 1 to 90 days.
- **Audit log:** retained approximately 365 days by default, configurable from 30 to 3,650 days for security, integrity, and legal/compliance purposes.
...
- Activities relevant to the data transferred: Provision of the Axel webhook-ingestion and event-pipeline Service, including edge ingestion, raw payload storage for replay, queuing, declarative route/filter/transform evaluation, delivery fan-out, logging, analytics, support, security, and billing.
...
**Nature and purpose of the Processing.** Receiving inbound webhook payloads at the Cloudflare edge; storing raw payloads in Cloudflare R2 for replay; queuing events; evaluating customer-authored declarative route, filter, and transform configuration; fanning out deliveries with stable delivery identifiers to the Customer's configured destinations (HTTP endpoints, MongoDB, Postgres, R2); and related logging, analytics, monitoring, support, security, and billing — all for the purpose of providing the Service to the Customer in accordance with the Terms and the Customer's configuration.
...
- Multi-tenant scoping: Postgres rows, ClickHouse records, object keys, and queue messages carry `workspace_id`, and dashboard and service queries require the active Workspace identifier and authorization context.
...
### 3. Application-layer route evaluation

- Customer route filters and transforms are evaluated as declarative configuration without `eval` or customer JavaScript. Route configuration is not given network, filesystem, host, or process primitives.
...
- Delivery attempts use stable identifiers to support duplicate suppression inside Axel and receiver-side deduplication for external HTTP destinations.
...
| # | Sub-processor | Purpose / processing activity | Categories of data processed | Location |
|---|---------------|-------------------------------|------------------------------|----------|
| 1 | Cloudflare, Inc. | Edge ingest, raw payload object storage (R2), and message queues | Full webhook payloads and associated metadata | United States, with global edge processing |
| 2 | Render (Render Services, Inc.) | Application compute (routing and delivery services, including outbound fan-out of payloads to destinations), PostgreSQL control database, and self-hosted ClickHouse analytics/log database | Account and configuration data, delivery logs, payload-derived metadata, and webhook payload contents processed in transit during outbound delivery / fan-out | United States |
| 3 | Vercel Inc. | Hosting for the customer dashboard and marketing site | Account session data and request metadata | United States |
