...
/**
 * Cookie-consent plumbing for the marketing site.
 *
...
export const CONSENT_REQUIRED_COOKIE = "axel_consent_required";
/** The visitor's stored choice. */
export const CONSENT_DECISION_COOKIE = "axel_cookie_consent";
/** Window event the footer "Cookie preferences" control dispatches to reopen the banner. */
...
export function enableAnalytics(): void {
  ...
  if (!started) {
    posthog.init(key, {
      ...
      // Pins SDK defaults: pageviews captured on route changes, pageleave on.
      defaults: "2026-01-30",
      // Belt-and-suspenders: start opted-out so a stray re-init can never
      // capture before the explicit opt_in below.
 ...
 * `instrumentation-client.ts`, so returning consented visitors start analytics
...
export function bootstrapAnalytics(): void {
  ...
  // No decision yet: outside consent-required regions, rely on legitimate
  // interest. Inside them (or unknown geo), wait for the banner.
...
export function consentDecisionPending(): boolean {
  ...
  // Missing required-cookie (e.g. middleware didn't run) is treated as required.
