    ...
    void resolveAll(hostname).then(
      ...
      (error: unknown) => {
        callback(error instanceof Error
          ? error
          : lookupError(`ssrf_blocked: ${hostname} could not be resolved`, "ENOTFOUND"), "", 0);
      },
    );
  };
}
  ...
  socket.connect = ((
    ...
    if (typeof portOrOptions === "number") {
      ...
      if (isPrivateOrUnsafeIp(host)) {
        queueMicrotask(() => socket.destroy(lookupError(
    ...
    if (typeof portOrOptions === "string" || "path" in portOrOptions) {
      queueMicrotask(() => socket.destroy(lookupError(
        "ssrf_blocked: local Unix socket destinations are not allowed",
        "ESSRF",
      )));
      return socket;
    }
