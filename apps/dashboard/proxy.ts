 ...
 * making pages dynamic. Set once per visitor (skipped when already present).
 *
...
export function proxy(req: NextRequest): NextResponse {
  ...
  if (req.cookies.has(COOKIE)) return NextResponse.next(forwarded);
...
export const config = {
  // Pages only — skip static assets, API/cron routes, the `/ingest` proxy, files.
