...
configured signature or Basic authentication. New integrations must not put
credentials in URLs: the receiving application cannot remove them from upstream
provider, proxy, browser, or platform access logs.
...
If a sender cannot supply either, design and test an authenticated adapter before
ending compatibility; do not treat the deadline alone as successful migration.
