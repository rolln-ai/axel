...
1. **Edge ingest** — Cloudflare Worker. Validates source token, applies source caps/rate limits, generates `event_id`, puts the raw body in R2, sends a compact message to a sharded queue, returns 202.
