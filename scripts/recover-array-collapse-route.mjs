...
const fail = () => { throw new Error("route_recovery_precondition_failed"); };
...
async function loadState(client, workspaceId, routeId, lock = false) {
  ...
  if (!route || route.status !== "errored" || route.error_reason !== REASON || !route.graph) fail();
  ...
  if (destinations.length !== 1 || destinations.some(d => d.status !== "active" || d.delivery_paused || d.circuit_state !== "closed")) fail();
  ...
  if (failures.length === 0 || failures.length > 20 || failures.some(f => f.destination_id && f.destination_id !== destinations[0].id)) fail();
  return { route, destinations, graph, failures };
}

export async function recoverArrayCollapseRoute(client, options) {
  const { workspaceId, routeId, expectedUpdatedAt, readPayload, runUrl } = options;
  ...
  if (Date.parse(before.route.updated_at) !== Date.parse(expectedUpdatedAt)) fail();
  for (const row of before.failures) {
    ...
    if (executeGraph(payload, before.graph).deliveries.length !== 1) fail();
  }
  ...
  try {
    ...
    if (JSON.stringify(current) !== JSON.stringify(before)) fail();
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { console.error("route_recovery_failed_inspect_current_state_before_retry"); process.exitCode = 1; });
}
